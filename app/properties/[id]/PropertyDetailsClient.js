...
import { FaPhone, FaEnvelope, FaUser, FaMapMarkerAlt, FaBed, FaBath, FaWhatsapp, FaShare, FaHeart, FaRegHeart, FaLock, FaBuilding } from 'react-icons/fa';
import { FaLocationDot, FaRulerCombined } from 'react-icons/fa6';
import { maskEmail, maskPhone } from '@/lib/contactUtils';
...
export default function PropertyDetailsClient({ initialProperty }) {
  ...
  useEffect(() => {
    if (!loading && user && property.owner) {
      setIsOwner(user._id === (typeof property.owner === "string" ? property.owner : property.owner._id));
    } else {
      setIsOwner(false);
    }
  }, [user, loading, property.owner]);
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-[20px] shadow-lg p-6 space-y-6">
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="flex flex-col items-center text-center p-4 bg-gray-50 rounded-xl">
                  <FaRulerCombined className="text-[#ffa509] text-2xl mb-2" />
                  ...
                  <div className="space-y-3">
                    ...
                    {user ? (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="font-medium">
                              Primary: {user.subscription?.status === 'active' 
                                ? (property.owner.phone?.primary || property.owner.phone)
                                : maskPhone(property.owner.phone?.primary || property.owner.phone)}
                            </span>
                            {property.owner.phone?.secondary && (
                              <span className="text-sm text-gray-600">
                                Secondary: {user.subscription?.status === 'active'
                                  ? property.owner.phone.secondary
                                  : maskPhone(property.owner.phone.secondary)}
                              </span>
                            )}
                          </div>
                        </div>
                            ...
                            <div className="flex">
                              ...
                              <div className="ml-3">
                                <p className="text-sm text-yellow-700">
                                  Subscribe to view full contact details and unlock all property owner information.
                                </p>
                                <div className="mt-2">
                                  <Link
                                    href={`/subscription?returnUrl=/properties/${property._id}&propertyId=${property._id}`}
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl text-center">
                        <p className="text-gray-600 mb-3">Contact information is available to registered users</p>
                        <Link 
                          href="/login" 
                          className="inline-block bg-[#ffa509] text-white px-4 py-2 rounded-lg hover:bg-[#ff9100] transition-colors"
                        >
