...
import { FaInbox, FaPaperPlane, FaClock, FaCheck, FaArchive, FaEnvelope, FaPhone, FaCalendarAlt, FaExclamationCircle, FaReply, FaFilter } from 'react-icons/fa';
...
export default function EnquiryManagement({ initialFilter = null } = {}) {
  const { user } = useAuth();
  ...
  const getFilteredEnquiries = () => {
    const baseEnquiries = activeTab === "received" ? enquiries.received : enquiries.sent;
    if (statusFilter === "all") {
      return baseEnquiries;
    }
    return baseEnquiries.filter(enquiry => enquiry.status === statusFilter);
  };

  const displayEnquiries = getFilteredEnquiries();
          ...
          {displayEnquiries?.length > 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-6">
                      ...
                      <div className="space-y-3">
                        <p className="text-gray-300 flex items-center">
                          ...
                          <span className="ml-2">{enquiry.sender?.phone?.primary || "N/A"}</span>
                        </p>
                        {enquiry.sender?.phone?.secondary && (
                          <p className="text-gray-300 flex items-center">
                            <FaPhone className="mr-2 text-gray-400" />
                            <span className="font-semibold">Secondary Phone: </span>
                            <span className="ml-2">{enquiry.sender.phone.secondary}</span>
                          </p>
                        )}
            ...
            <div className="text-center py-16 bg-white/10 backdrop-blur-lg rounded-2xl">
              ...
              <p className="text-gray-300 mt-2">
                {statusFilter === "all" 
                  ? `Your ${activeTab} enquiries will appear here`
